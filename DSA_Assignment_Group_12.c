...
uint64_t hilbertXYToIndex(uint64_t n, uint64_t x, uint64_t y)
{
    ...
    for (int64_t s = n / 2; s >= 1; s /= 2)
    {
        uint64_t rx = (x & mask) >> (s - 1);
        uint64_t ry = (y & mask) >> (s - 1);

        uint64_t index = (rx << 1) | ry;

        switch (index)
        {
        case 0: // A transformation
        {
            uint64_t temp = x;
            x = y;
            y = temp;
        ...
        break;
        case 1: // B transformation
        {
            uint64_t temp = x ^ y ^ mask;
            x = x ^ temp;
            y = y ^ temp;
        }
        break;
        case 2: // C transformation
        {
            uint64_t temp = x ^ y;
            x = x ^ temp;
            y = y ^ temp;
    ...
    return ((interleave(i1) << 1) | interleave(i0)) >> (64 - n);
}
...
Rectangle new_rectangle(int bottomLeft_X, int bottomLeft_y, int topRight_x, int topRight_y)
{
    ...
    mbr.h = xy2d(20, (mbr.bottom_left.x + mbr.top_right.x) / 2, (mbr.bottom_left.y + mbr.top_right.y) / 2);
    return mbr;
}
